...
public class Render {
	...
	public void renderImage() {
		for (int i = 0; i < _imageWriter.getNx(); i++)
			for (int j = 0; j < _imageWriter.getNy(); j++) {
				Ray ray = _scene.getCamera().constructRayThroughPixel(_imageWriter.getNx(), _imageWriter.getNx(), i, j,
	...
	public void renderImageSuperSampling() {
		...
		for (int i = 0; i < _imageWriter.getNx(); i++) {
			for (int j = 0; j < _imageWriter.getNy(); j++) {
				...
				for (int g = 0; g < n; g++) {
					for (int d = 0; d < n; d++) {
						Ray ray1 = _scene.getCamera().constructRayThroughPixel(_imageWriter.getNx(),
								_imageWriter.getNx(), i + (g + 0.5) / n, j + (g + 0.5) / n, _scene.getDistance(),
								_imageWriter.getWidth(), _imageWriter.getHeight());
						GeoPoint closestPoint1 = findClosestIntersection(ray1);
						c = c.add(closestPoint1 == null ? _scene.getBackground() : calcColor(closestPoint1, ray1));
	...
	public Color calcColor(GeoPoint geopoint, Ray inRay) {
		return calcColor(geopoint, inRay, MAX_CALC_COLOR_LEVEL, 1.0).add(_scene.getAmbient().getIntensity());
	}
